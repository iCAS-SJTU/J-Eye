...
//
      ...
      /* If the patient is out of the robot reachable distance, warnign message will be written on the screen*/
      if (prediction->bbox.x < 600 || prediction->bbox.x + prediction->bbox.width > 1320 || prediction->bbox.y + prediction->bbox.height > 800) {
        convert_rgb_to_yuv_clrs (kpriv->label_color, &yScalar, &uvScalar);
        putText (frameinfo->lumaImg, "Please stand in the screen center", cv::Point (90, 90), kpriv->font, kpriv->font_size,
            Scalar (yScalar), 1, 2);
        putText (frameinfo->chromaImg, "Please stand in the screen center", cv::Point (45, 45), kpriv->font,
            kpriv->font_size / 2, Scalar (uvScalar), 1, 2);

        rectangle (frameinfo->lumaImg, Point (600, 280), Point (1320, 800), Scalar (yScalar), kpriv->line_thickness, 1, 0);
        rectangle (frameinfo->chromaImg,Point (300, 140), Point (660, 400), Scalar (uvScalar), kpriv->line_thickness, 1, 0);
      }
      // else if (prediction->bbox.width / (float)prediction->bbox.height < 0.64){
      else {
        ...
        /*Calculate for arm position. Assume that people must turn left on the screen*/        
        int center_x = prediction->bbox.x + 1.25 * prediction->bbox.width; 
        int center_y = prediction->bbox.y + 2 * prediction->bbox.height; 

        circle(frameinfo->lumaImg, Point(center_x, center_y), prediction->bbox.width / 6, Scalar(yScalar), -1, 1);
        circle(frameinfo->lumaImg, Point(center_x, center_y), prediction->bbox.width / 6, Scalar(uvScalar), -1, 1);

        /* Mask detection*/
        int detect_point1_y = prediction->bbox.y + 0.2 * prediction->bbox.height; // reference point
        int detect_point1_x = prediction->bbox.x + 0.5 * prediction->bbox.width;
        int detect_point2_y = prediction->bbox.y + 0.8 * prediction->bbox.height; // mask position point
        int detect_point2_x = prediction->bbox.x + 0.5 * prediction->bbox.width;

        int point1_y = frameinfo->lumaImg.at<uchar>(detect_point1_y, detect_point1_x);
        int point1_uv = frameinfo->chromaImg.at<uchar>(detect_point1_y / 2, detect_point1_x / 2);
        int point2_y = frameinfo->lumaImg.at<uchar>(detect_point2_y, detect_point2_x);
        int point2_uv = frameinfo->chromaImg.at<uchar>(detect_point2_y / 2, detect_point2_x / 2);

        if (point1_y > point2_y) {
            putText (frameinfo->lumaImg, "Please wear your mask", cv::Point (90, 90), kpriv->font, kpriv->font_size,
                Scalar (yScalar), 1, 2);
            putText (frameinfo->chromaImg, "Please wear your mask", cv::Point (45, 45), kpriv->font,
                kpriv->font_size / 2, Scalar (uvScalar), 1, 2);
        }
        // Vec3b frame_rgb = frameinfo->image.at<Vec3b>(detect_point_y, detect_point_x);
  
        
        // if ( frame_b < 220 || frame_r / (float)frame_b > 0.6 || frame_g < 160) {
        //     putText (frameinfo->lumaImg, "Please wear your mask", cv::Point (90, 90), kpriv->font, kpriv->font_size,
        //         Scalar (yScalar), 1, 2);
        //     putText (frameinfo->chromaImg, "Please wear your mask", cv::Point (45, 45), kpriv->font,
        //         kpriv->font_size / 2, Scalar (uvScalar), 1, 2);
